/**
* @license ap-fullscreen-modal.js v0.1
* Updated: 28.04.2016
* {DESCRIPTION}
...
;(function($) {
	...
	ApFullscreenModal.prototype = {
		...
		_addContainer: function() {
			this.$container = $(ApFullscreenModal.template).appendTo($('body'));
		...
		_bind: function() {
			...
			if (this.settings.closeSelector) {
				this.$container.find(this.settings.closeSelector).on('click.' + eventNamespace, function() {
					self.close();
					return false;
				});
			}
		...
		open: function(sender) { // sender not used at the moment
			if (this.opened) {
				return;
			}
			this.opened = true;
			...
			this.$container.addClass(cssPrefix + 'open');
			this.$content.focus();
		...
		close: function() {
			var self = this;
			if (this.opened) {
				if (this._triggerHandler('close') !== false) {
					this.$container.removeClass(cssPrefix + 'open');
					setTimeout(function() {
	...
	$.fn.apFullscreenModal = function( options ) {
		if (typeof options === 'string') {
			...
			var instance = new ApFullscreenModal(this, options);
